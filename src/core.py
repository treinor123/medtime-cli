...
class MedTimeCore:
    ...
    def adicionar_medicamento(self, nome: str, horario: str, dosagem: str) -> dict:
        ...
        # Validação de campos vazios (Requisito de Entrada Inválida)
        if not nome.strip() or not horario.strip() or not dosagem.strip():
            raise ValueError("Todos os campos são obrigatórios.")
        
        # Validação do formato da hora (Requisito de Caso Limite)
        ...
        # Faz o INSERT real no banco na nuvem
    ...
    def remover_medicamento(self, med_id: int) -> bool:
        ...
        resposta = self.db.table("medicamentos").delete().eq("id", med_id).execute()
